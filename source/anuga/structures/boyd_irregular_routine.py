...
class Boyd_box_culvert_routine(culvert_routine.Culvert_routine):
    ...
    def __init__(self, structure, manning=0.0):

        culvert_routine.Culvert_routine.__init__(self, culvert, manning)
        
        self.slice_coordinates = structure.get_slice_coordinates()
    ...
    def __call__(self):
        ...
        if self.inflow.get_average_height() > 0.01: #this value was 0.01:
            ...
            for  i in len(self.slice_coordinates):
                for j in len(self.slice_coordinates[i]):
                    
                    slice_invert = self.slice_coordinates[i][j][1]
                    slice_obvert = self.slice_coordinates[i][j][2]
                    
                    if self.outflow.get_average_height() > slice_obvert: # The slice is Submerged
                
                        #if self.outflow.get_average_height() > slice_invert: # The invert is wet
