...
##-----------------------------------------------------------------------
    ...
    if not parallel:
        ...
        for i in range(samples):
            if tri_ids[i] >= 0:                
                success = success and num.allclose(control_data[i], stage.centroid_values[tri_ids[i]])
                if verbose: 
                    print 'P%d tri %d, control = %s, actual = %s, Success = %s' %(myid, i, control_data[i], stage.centroid_values[tri_ids[i]], success) 
