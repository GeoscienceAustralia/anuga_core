...
# If this is commented out, the code works. Otherwise the interpolated
# timeseries are wrong - even when run on one processor.
...
for i, (x,y) in enumerate(interpolation_points):
    gauge_values[i] = gauge_values[i][:51] # In case an extra step was taken
...
msg = 'Computed time series and reference time series are different: %s'\
      %(gauge_values[0]-G0)
assert allclose(gauge_values[0], G0), msg

msg = 'Computed time series and reference time series are different: %s'\
      %(gauge_values[1]-G1)
assert allclose(gauge_values[1], G1), msg

msg = 'Computed time series and reference time series are different: %s'\
      %(gauge_values[2]-G2)
assert allclose(gauge_values[2], G2), msg

msg = 'Computed time series and reference time series are different: %s'\
      %(gauge_values[3]-G3)
assert allclose(gauge_values[3], G3), msg        
