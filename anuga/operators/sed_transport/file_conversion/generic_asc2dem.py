...
def generic_asc2dem(name_in, name_out=None, quantity_name=None,
    ...
    if use_cache is True:
        ...
        result = cache(_generic_convert_dem_from_ascii2netcdf, name_in, kwargs,
                       dependencies=[name_in,
                                     name_in[:-4] + '.prj'],
                       verbose=verbose)

    else:
        result = apply(_generic_convert_dem_from_ascii2netcdf, [name_in], kwargs)

    return result


def _generic_convert_dem_from_ascii2netcdf(name_in, name_out = None,
