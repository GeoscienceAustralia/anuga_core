                            ...
                            long low_froude)
{
  ...
  */
  ...
  soundspeed_left = sqrtl(g * h_left);
  soundspeed_right = sqrtl(g * h_right);
  // soundspeed_left  = sqrtl(g*hle);
  // soundspeed_right = sqrtl(g*hre);
  ...
  {
    local_fr = sqrtl((u_right * u_right + u_left * u_left + v_right * v_right + v_left * v_left) /
                    (soundspeed_left * soundspeed_left + soundspeed_right * soundspeed_right + 1.0e-10));
    local_fr = sqrtl(fmin(1.0, 0.01 + fmax(local_fr - 0.01, 0.0)));
  }
  else
  {
    local_fr = 1.0;
                                   ...
                                   double *max_speed_local)
{
  ...
  rw = Qfactor * twothirds * maxhd * sqrtl(twothirds * g * maxhd);
  // Factor for villemonte correction
  rw2 = Qfactor * twothirds * minhd * sqrtl(twothirds * g * minhd);
  // Useful ratios
  ...
  {
    *max_speed_local = sqrtl(g * (maxhd + weir_height)) + fabs(edgeflux[0] / (maxhd + 1.0e-12));
  }
