...
#===================================
       if POT2 != F0:
           ...
           DU[7]= ALP1*(old_div(S2D,R3)-old_div(F3*Y*S,R5)) +ALP2*(old_div(F3*Y,R5)*VY+P*QR)
           DU[8]=-ALP1*(old_div(C2D,R3)-old_div(F3*Y*T,R5)) +old_div(ALP2*F3*D,R5)*VY
           DU[9]=                          old_div(ALP2*F3*X,R5)*VZ
           DU[10]= ALP1*(old_div(C2D,R3)+old_div(F3*D*S,R5)) +old_div(ALP2*F3*Y,R5)*VZ
           DU[11]= ALP1*(old_div(S2D,R3)-old_div(F3*D*T,R5)) +ALP2*(old_div(F3*D,R5)*VZ-P*QR)
           for I in range(0,12):
...
#========================================
       if POT3!=F0:
           ...
           DU[7]= ALP1*(old_div(C2D,R3)-old_div(F3*Y*T,R5)) -ALP2*(Y*WY+Q)*QR
           DU[8]= ALP1*(old_div(S2D,R3)-old_div(F3*Y*S,R5)) -ALP2*D*QR*WY
           DU[9]= old_div(ALP1*F3*X*D,R5)          -ALP2*X*QR*WZ
           DU[10]=-ALP1*(old_div(S2D,R3)-old_div(F3*D*T,R5)) -ALP2*Y*QR*WZ
           DU[11]= ALP1*(old_div(C2D,R3)+old_div(F3*D*S,R5)) -ALP2*(D*WZ-Q)*QR
           for I in range(0,12):
...
#======================================
      if  POT1!=F0:
        ...
        DU[6]= DU[4]
        DU[7]= old_div(F3*X,R5)*( ALP4*B5*CD -old_div(ALP5*F5*C,R2)*(F2*Y*SD+Q*B7) )
        DU[8]= old_div(F3*X,R5)*(-ALP4*B5*SD +old_div(ALP5*F5*C,R2)*(D*B7*SD-Y*C7*CD) )
        DU[9]= old_div(F3,R5)*   (-ALP4*D*A5*CD +ALP5*C*(A5*CD+D*QR5*A7) )
        DU[10]= old_div(F15*X,R7)*( ALP4*Y*D*CD  +ALP5*C*(D*B7*SD-Y*C7*CD) )
...
#===================================
      if POT2!=F0:
        ...
        DU[1]=old_div(-ALP4,R3)*(C2D-old_div(F3*Y*T,R2)) +old_div(ALP5*F3*C,R5)*(S-Y*P*QR5)
        DU[2]=old_div(-ALP4*A3,R3)*SDCD         +old_div(ALP5*F3*C,R5)*(T+D*P*QR5)
        DU[3]= old_div(ALP4*F3*T,R5)*A5              -old_div(ALP5*F5*C*P*QR,R2)*A7
        DU[4]= old_div(F3*X,R5)*(ALP4*(C2D-old_div(F5*Y*T,R2))-old_div(ALP5*F5*C,R2)*(S-Y*P*QR7))
        DU[5]= old_div(F3*X,R5)*(ALP4*(F2+A5)*SDCD   -old_div(ALP5*F5*C,R2)*(T+D*P*QR7))
        DU[6]= DU[4]
        DU[7]= (old_div(F3,R5)*(ALP4*(F2*Y*C2D+T*B5)
                               +ALP5*C*(S2D-old_div(F10*Y*S,R2)-P*QR5*B7)))
        DU[8]= old_div(F3,R5)*(ALP4*Y*A5*SDCD-ALP5*C*((F3+A5)*C2D+Y*P*DR5*QR7))
        DU[9]= old_div(F3*X,R5)*(-ALP4*(S2D-T*DR5) -old_div(ALP5*F5*C,R2)*(T+D*P*QR7))
        DU[10]= (old_div(F3,R5)*(-ALP4*(D*B5*C2D+Y*C5*S2D)
                                -ALP5*C*((F3+A5)*C2D+Y*P*DR5*QR7)))
        DU[11]= old_div(F3,R5)*(-ALP4*D*A5*SDCD-ALP5*C*(S2D-old_div(F10*D*T,R2)+P*QR5*C7))
        for I in range(0,12):
            U[I]=U[I]+old_div(POT2,PI2)*DU[I]
#========================================
#=====  TENSILE-FAULT CONTRIBUTION  =====
#========================================
      if POT3!=F0:
        ...
        DU[1]= old_div(ALP4,R3)*(S2D-old_div(F3*Y*S,R2))+old_div(ALP5*F3,R5)*(C*(T-Y+Y*Q*QR5)-Y*Z)
        DU[2]=old_div(-ALP4,R3)*(F1-A3*SDSD)   -old_div(ALP5*F3,R5)*(C*(S-D+D*Q*QR5)-D*Z)
        DU[3]=old_div(-ALP4*F3*S,R5)*A5 +ALP5*(C*QR*QR5*A7-old_div(F3*Z,R5)*A5)
...
#======================================
      if DISL1!=F0:
        ...
        DU[8]=old_div(-ALP4*Q,R3)+YY0*SD  +ALP5*(CDR*CD+C*D*QR-(Y0*CD+Q*Z0)*SD)
        DU[9]= ALP4*XI*PPZ*CD    -ALP5*XI*QQZ
        DU[10]= ALP4*F2*(old_div(Y,R3)-Y0*CD)*SD+old_div(D,R3)*CD -ALP5*(CDR*CD+C*D*QR)
...
#======================================
      if DISL2!=F0 :
        ...
        DU[5]=    old_div(D,R3)-Y0*SD +old_div(ALP5*C,R3)*(F1-old_div(F3*Q2,R2))
...
#========================================
      if DISL3!=F0:
        ...
        DU[3]= old_div(ALP4*XI,R3)*SD+XI*Q*Y32*CD+ALP5*XI*(old_div(F3*C*ET,R5)-F2*Z32-Z0)
        DU[4]= ALP4*F2*Y0*SD-old_div(D,R3) +old_div(ALP5*C,R3)*(F1-old_div(F3*Q2,R2))
        DU[5]=-ALP4*YY0           -ALP5*(C*ET*QR-Q*Z0)
        DU[6]= ALP4*(old_div(Q,R3)+Y0*SDCD)   +ALP5*(old_div(Z,R3)*CD+C*D*QR-Q*Z0*SD)
        ...
        DU[9]=  old_div(-ET,R3)+Y0*CDCD -ALP5*(old_div(Z,R3)*SD-C*Y*QR-Y0*SDSD+Q*Z0*CD)
