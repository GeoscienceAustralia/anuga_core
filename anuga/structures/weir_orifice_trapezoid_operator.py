...
def weir_orifice_trapezoid_function(width, 
    ...
    if blockage >= 1.0:
        ...
        Q_inlet_unsubmerged = 1.7*((2*(1-blockage)*width+(1-blockage)*depth*(z1+z2))/2)*driving_energy**1.50 # Flow based on Inlet Ctrl Inlet Unsubmerged (weir flow equation)
        Q_inlet_submerged = 0.8*anuga.g**0.5*(0.5*(1-blockage)*depth*(2*(1-blockage)*width+(1-blockage)*depth*(z1+z2)))*driving_energy**0.5  # Flow based on Inlet Ctrl Inlet Submerged (orifice equation)
        ...
        if Q_inlet_unsubmerged < Q_inlet_submerged:
            ...
            while abs(dyc)>0.00001:
                Tc=(1-blockage)*width+(z1+z2)*dcrit
                Pc=(1-blockage)*width+((z1**2+1)**0.5+(z2**2+1)**0.5)*dcrit
                Ac=0.5*dcrit*((1-blockage)*width+Tc)
                Rc=Ac/Pc
            ...
            if dcrit > depth:
                dcrit = depth
                flow_area = (1-blockage)*width*dcrit+0.5*(z1+z2)*dcrit**2
                perimeter= 2.0*(1-blockage)*width+(z1+z2)*dcrit + (dcrit**2+(z1*dcrit)**2)**0.5 + (dcrit**2+(z2*dcrit)**2)**0.5
            else: # dcrit < depth
                flow_area = (1-blockage)*width*dcrit+0.5*(z1+z2)*dcrit**2
                perimeter= (1-blockage)*width + (dcrit**2+(z1*dcrit)**2)**0.5 + (dcrit**2+(z2*dcrit)**2)**0.5
        ...
        if outlet_culvert_depth > depth:
            ...
            flow_area = (1-blockage)*width*(1-blockage)*depth+0.5*(z1+z2)*((1-blockage)*depth)**2  # Cross sectional area of flow in the culvert
            perimeter = 2.0*(1-blockage)*width+(z1+z2)*(1-blockage)*depth + (((1-blockage)*depth)**2+(z1*((1-blockage)*depth))**2)**0.5 + (((1-blockage)*depth)**2+(z2*((1-blockage)*depth))**2)**0.5
            case = 'Inlet CTRL Outlet unsubmerged PIPE PART FULL'
        else:
            flow_area = (1-blockage)*width*outlet_culvert_depth+0.5*(z1+z2)*outlet_culvert_depth**2
            perimeter = 2.0*(1-blockage)*width+(z1+z2)*outlet_culvert_depth + (outlet_culvert_depth**2+(z1*outlet_culvert_depth)**2)**0.5 + (outlet_culvert_depth**2+(z2*outlet_culvert_depth)**2)**0.5
        ...
        if delta_total_energy < driving_energy:
            ...
            if outlet_enquiry_depth > depth:        # The Outlet is Submerged
                ...
                flow_area=(1-blockage)*width*(1-blockage)*depth+0.5*(z1+z2)*((1-blockage)*depth)**2  # Cross sectional area of flow in the culvert
                perimeter=(1-blockage)*width + (((1-blockage)*depth)**2+(z1*((1-blockage)*depth))**2)**0.5 + (((1-blockage)*depth)**2+(z2*((1-blockage)*depth))**2)**0.5
                ...
                while abs(dyn)>0.0001:
                    Tn=(1-blockage)*width+(z1+z2)*dnorm
                    An=0.5*dnorm*((1-blockage)*width+Tn)
                    Pn=(1-blockage)*width+2*dnorm*((z1**2+z2**2)**0.5)
                    Rn=An/Pn
                ...
                if outlet_culvert_depth > depth:
                    outlet_culvert_depth=depth
                    flow_area=(1-blockage)*width*(1-blockage)*depth+0.5*(z1+z2)*((1-blockage)*depth)**2
                    perimeter=(1-blockage)*width + (((1-blockage)*depth)**2+(z1*((1-blockage)*depth))**2)**0.5 + (((1-blockage)*depth)**2+(z2*((1-blockage)*depth))**2)**0.5
                    case = 'Outlet is Flowing Full'
                else:
                    flow_area=(1-blockage)*width*outlet_culvert_depth+0.5*(z1+z2)*outlet_culvert_depth**2
                    perimeter=(1-blockage)*width + (outlet_culvert_depth**2+(z1*outlet_culvert_depth)**2)**0.5 + (outlet_culvert_depth**2+(z2*outlet_culvert_depth)**2)**0.5
